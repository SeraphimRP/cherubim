...
class Announce(commands.Cog):
    ...
    @checks.admin()
    async def announcesend(self, ctx):
        """Sends the announce screen in the current channel."""
    ...
    @commands.command(no_pm=True)
    @checks.admin()
    async def announcelist(self, ctx):
        """Lists entries in the announce screen."""
        screen = self.get_screen(ctx.guild.id)
        if screen is None or screen.is_empty():
            await ctx.send('The announce screen is empty.')
        else:
            message = 'Entries in announce screen:\n'
            message += '\n'.join(
    ...
    @commands.command(no_pm=True)
    @checks.admin()
    async def announceadd(self, ctx, index=None):
        """Adds an entry."""
        ...
        if choice in ['t', 'text']:
            ...
            screen.add(announce_TEXT, index, text=text)
            await ctx.send('The text entry has been created.')
        elif choice in ['b', 'textbox']:
            screen.add(announce_TEXT_BOX, index)
    ...
    @commands.command(no_pm=True)
    @checks.admin()
    async def announceedit(self, ctx, index):
        """Edits an entry."""
        screen = self.get_screen(ctx.guild.id)
        index = await self._validate_index(ctx, screen, index)
        if index is not None:
    ...
    @commands.command(no_pm=True)
    @checks.admin()
    async def announceremove(self, ctx, index):
        """Removes an entry."""
        screen = self.get_screen(ctx.guild.id)
        index = await self._validate_index(ctx, screen, index)
        if index is not None:
    ...
    @commands.command(no_pm=True)
    @checks.admin()
    async def announcemove(self, ctx, index, new_index):
    ...
    @commands.command(no_pm=True)
    @checks.admin()
    async def announceswap(self, ctx, first_index, second_index):
